...
class Human:
    def __init__(self, name="Human", age=None, job=None, home=None, car=None):
        ...
        self.happiness = 50
        self.saturation = 50
        print(f"The humans name is {self.name}")
        logging.info(f'The humans name is {self.name}')

    def get_home(self):
        self.home = House()
        logging.info(f'{self.name} got a house')

    def get_car(self, brand_of_car):
        self.car = TheCar(brand_of_car)
        logging.info(f'{self.name} got a car')
    ...
    def go_to_job(self):
        if self.job:
            ...
            logging.info(f'{self.name} is going to work as a {self.job.title}.')
            ...
            logging.info(f'Money: {self.money}, Happiness: {self.happiness}, Saturation: {self.saturation}')
        else:
            print(f"{self.name} has no job to go to!")
            logging.info(f'{self.name} has no job to go to!')
    ...
    def eat_food(self, amount=10):
        ...
        logging.info(f'{self.name} ate food. Saturation is now {self.saturation}.')
    ...
    def relax(self, amount=10):
        ...
        logging.info(f'{self.name} is relaxing. Happiness is now {self.happiness}.')
...
class House:
    ...
    def clean_house(self):
        ...
        logging.info(f'The house has been cleaned. Cleanliness is now {self.mess}.')

    def stock_food(self, amount=10):
        self.food = min(100, self.food + amount)
        print(f"Food stocked. Current food level is {self.food}.")
        logging.info(f'Food stocked. Current food level is {self.food}.')
...
logging.info(f'Stats for {john.name}:')
logging.info(f'Money: {john.money}, Happiness: {john.happiness}, Saturation: {john.saturation}')
...
logging.info(f'{john.name}\'s car details:')
logging.info(f'Car Brand: {john.car.brand}, Fuel: {john.car.fuel}, Horsepower: {john.car.horsepower}')

print(f"\nFinal stats for {john.name}:")
print(f"Money: {john.money}, Happiness: {john.happiness}, Saturation: {john.saturation}")
logging.info(f'Final stats for {john.name}:')
logging.info(f'Money: {john.money}, Happiness: {john.happiness}, Saturation: {john.saturation}')
